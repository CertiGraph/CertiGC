...
Lemma sapi_ptr_val: forall p m n,
    ...
    (force_val (sem_add_ptr_int int_or_ptr_type Signed (offset_val (WORD_SIZE * m) p) (vint n)))
    = offset_val (WORD_SIZE * (m + n)) p.
Proof.
...
Lemma sapil_ptr_val: forall p m n,
    isptr p ->
    if Archi.ptr64 then
      force_val (sem_add_ptr_long int_or_ptr_type (offset_val (WORD_SIZE * m) p) (Vlong (Int64.repr n)))
      = offset_val (WORD_SIZE * (m + n)) p
    else
      force_val (sem_add_ptr_int int_or_ptr_type Signed (offset_val (WORD_SIZE * m) p) (vint n))
      = offset_val (WORD_SIZE * (m + n)) p.
...
Lemma data_at_mfs_eq: forall g v i sh nv,
    ...
    field_at sh int_or_ptr_type [] (Znth i (heapgraph_block_cells_vals g v)) (offset_val (WORD_SIZE * i) nv)
    = data_at sh (tarray int_or_ptr_type (i + 1)) (sublist 0 (i + 1) (heapgraph_block_cells_vals g v)) nv.
Proof.
  ...
  rewrite (data_at_tarray_value
             sh (i + 1) i nv
             (sublist 0 (i + 1) (heapgraph_block_cells_vals g v))
...
Proof.
  ...
  destruct H1 as [H1 [H7 [H8 [H9 H10]]]].
  ...
  destruct forward_p.
  {
    ...
    assert
      ...
      as H11
      by (rewrite <- (Zlength_map _ _ ((fun x y => Znth y x) (ti_args t_info))), <- H4, Zlength_map; trivial).
    pose proof (Znth_map _ (root2val g) _ H0) as H12.
    hnf in H0.
    rewrite H11 in H0.
    ...
    remember
      ...
      as root eqn:Heqroot.
    rewrite <- H11 in H0.
    pose proof (Znth_In _ _ H0) as H13.
    rewrite <- Heqroot in H13.
    rewrite H11 in H0.
    unfold Inhabitant_val in H12.
    assert
      (forall v, In (inr v) roots -> isptr (heapgraph_block_ptr g v))
      as H14.
    {
      intros.
      destruct H5 as [H5 H15].
      unfold heapgraph_block_ptr.
      red in H15.
      rewrite Forall_forall in H15.
      rewrite (filter_sum_right_In_iff v roots) in H14.
      apply H15 in H14.
    ...
    assert
      (is_pointer_or_integer (root2val g root))
      as H15.
    {
      destruct root as [[? | ?] | ?]; simpl; auto.
      - now destruct g0.
      - specialize (H14 _ H13).
        now apply isptr_is_pointer_or_integer.
    }
    assert
      (0 <= Znth z (live_roots_indices f_info) < MAX_ARGS)
      as H16
      by (apply (fi_index_range f_info), Znth_In; assumption).
    forward; rewrite H12.
    {
      entailer!.
    }
    assert_PROP
      (valid_int_or_ptr (root2val g root))
      as H17.
    {
      gather_SEP (graph_rep _) (outlier_rep _).
      sep_apply (root_valid_int_or_ptr _ _ _ _ H13 H5).
      entailer!.
    }
    forward_call (root2val g root).
    remember
      (graph_rep g * heap_rest_rep (ti_heap t_info) * outlier_rep outlier)
      as P eqn:HeqP.
    pose proof (graph_and_heap_rest_data_at_ _ _ _ H7 H) as H18.
    unfold generation_data_at_ in H18.
    remember
      (heapgraph_generation_base g from)
      as fp eqn:Heqfp.
    remember
      (heapgraph_generation_sh g from) as fsh.
    remember
      (gen_size t_info from - space_remembered (nth_space t_info from))
      as gn eqn:Heqgn.
    remember
      (WORD_SIZE * gn)%Z
      as fn eqn:Heqfn.
    assert (P |-- (weak_derives P (memory_block fsh fn fp * TT) && emp) * P) as H19.
    {
      ...
      pose proof (space_remembered__order (nth_space t_info from)) as H19.
      rewrite sizeof_tarray_int_or_ptr ; try lia.
      entailer!.
    }
    ...
    {
      unfold GC_Pointer2val.
      destruct g0.
      forward_if.
      2: {
        exfalso.
        now apply Int.one_not_zero in H20.
      }
      forward_call (Vptr b i).
      gather_SEP (graph_rep _) (heap_rest_rep _) (outlier_rep _).
      rewrite <- HeqP.
      destruct H5 as [H5 H21].
      replace_SEP 0
        ((weak_derives P (memory_block fsh fn fp * TT) && emp) * P)
        by (entailer; assumption).
      clear H19.
      Intros.
      simpl root2val in *.
      assert
        (P |-- (weak_derives P (valid_pointer (Vptr b i) * TT) && emp) * P)
        as H19.
      {
      ...
      replace_SEP 1
        ((weak_derives P (valid_pointer (Vptr b i) * TT) && emp) * P)
        by (entailer; assumption).
      Intros.
      clear H19.
      ...
      * rewrite HeqP. Intros.
        ...
        sep_apply H18.
        rewrite Heqfn in v.
        sep_apply (roots_outlier_rep_single_rep _ _ _ H13 H5).
        Intros.
        ...
        change
          (generation_sh (heapgraph_generation g from))
          with (heapgraph_generation_sh g from)
          in H19.
    ...
    Intros.
    rewrite <- Heqfp, <- Heqgn, <- Heqfn in H20.
    destruct vv.
    2: {
      forward_if.
      {
    ...
    rewrite v.
    assert
      (readable_share (heapgraph_generation_sh g from))
      as H20
      by (unfold heapgraph_generation_sh; apply writable_readable, generation_sh__writable).
    sep_apply (
      data_at_minus1_address
        (heapgraph_generation_sh g from)
        (Z2val (heapgraph_block_header g a))
    ...
    replace_SEP 0
      (vertex_rep (heapgraph_generation_sh g (addr_gen a)) g a)
      by (unfold vertex_rep, vertex_at; entailer!).
    unlocalize [graph_rep g].
    {
      apply (graph_vertex_ramif_stable _ _ H19).
    }
    forward_if.
    {
      try apply Int64.same_if_eq in H21.
      pose proof (heapgraph_block_header__repr_iff g a).
      simpl in H22.
      rewrite H22 in H21.
      clear H22.
      localize [vertex_rep (heapgraph_generation_sh g (addr_gen a)) g a].
      ...
      unfold vertex_rep, vertex_at.
      Intros.
      unfold heapgraph_block_cells_vals at 2.
      rewrite H21.
      assert
        (0 <= 0 < Zlength (heapgraph_block_cells_vals g a))
        as H22.
      {
      ...
      assert
        (is_pointer_or_integer (heapgraph_block_ptr g (block_copied_vertex (heapgraph_block g a))))
        as H23.
      {
      ...
      {
        unfold vertex_rep, vertex_at.
        unfold heapgraph_block_cells_vals at 3.
        rewrite H21.
        entailer!.
      }
      unlocalize [graph_rep g].
    ...
    sep_apply (graph_vertex_ramif_stable _ _ H19).
    Intros.
    freeze [1; 2; 3; 4; 5] FR.
    rewrite v.
    remember (heapgraph_generation_sh g from) as shv.
    assert
      (writable_share (space_sh sp_to))
      as H28
      by (rewrite <- H24; apply generation_sh__writable).
    remember (space_sh sp_to) as sht.
    rewrite (data_at__tarray_value _ _ 1).
    2: {
    ...
    assert_PROP (
      force_val (sem_add_ptr_int
                  ...
                  (offset_val (WORD_SIZE * (space_allocated sp_to + 1)) (space_base sp_to))
    ...
    forward.
    sep_apply (field_at_data_at_cancel sht (if Archi.ptr64 then tulong else tuint) (Z2val (heapgraph_block_header g a)) v0).
    clear H29.
    subst v0.
    rewrite offset_offset_val.
    replace
      (heapgraph_block_size g a - 1)
      with (Zlength (block_fields (heapgraph_block g a)))
      by (unfold heapgraph_block_size; lia).
    replace
      (WORD_SIZE * space_allocated sp_to + WORD_SIZE * 1)
      with (WORD_SIZE * (space_allocated sp_to + 1))%Z
      by rep_lia.
    remember (offset_val (WORD_SIZE * (space_allocated sp_to + 1)) (space_base sp_to)) as nv eqn:Heqnv.
    thaw FR.
    freeze [0; 1; 2; 3; 4; 5] FR.
    rename i into j.
    remember
      (Zlength (block_fields (heapgraph_block g a)))
      as n.
    assert
      (isptr nv)
      as H29
      by (subst nv; rewrite isptr_offset_val; assumption).
    remember
      (field_address thread_info_type [ArraySubsc (Znth z (live_roots_indices f_info)); StructField _args] ti)
      as p_addr eqn:Heqp_addr.
    remember
      (field_address heap_type [StructField _next; ArraySubsc (Z.of_nat to); StructField _spaces] (ti_heap_p t_info))
      as n_addr eqn:Heqn_addr.
    forward_for_simple_bound
      ...
      SEP
        ( vertex_rep shv g a
        ; data_at sht (tarray int_or_ptr_type i) (sublist 0 i (heapgraph_block_cells_vals g a)) nv
        ; data_at_ sht (tarray int_or_ptr_type (n - i)) (offset_val (WORD_SIZE * i) nv); FRZL FR
        ))%assert.
    {
      pose proof (block_fields__range2 (heapgraph_block g a)) as H30.
      simpl in H30.
    ...
    }
    {
      ...
      rewrite data_at_zero_array_eq by easy.
      entailer!.
    }
    {
      ...
      rewrite data_at_mfs_eq ; subst n ; try easy.
      entailer!.
    }
    thaw FR.
    rewrite v, <- Heqshv.
    ...
    assert
      (nv = heapgraph_block_ptr g (new_copied_v g to))
      as H30.
    {
    ...
    replace_SEP 0 (
      vertex_at
        ...
        (heapgraph_block_header g a)
        (heapgraph_block_cells_vals g a)
    ).
    {
    ...
    freeze [1; 2; 3; 4] FR.
    remember (lgraph_add_copied_v g a to) as g'.
    assert
      (heapgraph_block_ptr g' a = heapgraph_block_ptr g a)
      as H31
      by (subst g'; apply lacv_heapgraph_block_ptr_old; assumption).
    assert
      (heapgraph_block_ptr g' (new_copied_v g to) = heapgraph_block_ptr g (new_copied_v g to))
      as H32
      by (subst g'; apply lacv_heapgraph_block_ptr_new; assumption).
    rewrite <- H31.
    rewrite <- H32 in H30.
    assert
      (writable_share (heapgraph_generation_sh g' (addr_gen a)))
      as H33
      by (unfold heapgraph_generation_sh; apply generation_sh__writable).
    assert
      (heapgraph_has_block g' (new_copied_v g to))
      as H34
      by (subst g'; apply lacv_heapgraph_has_block_new; assumption).
    sep_apply (graph_rep_valid_int_or_ptr _ _ H34).
    Intros.
    rewrite <- H30 in H35.
    assert
      (heapgraph_has_block g' a)
      as H36
      by (subst g'; apply lgraph_add_copied_v__heapgraph_has_block; assumption).
    ...
    unfold vertex_rep, vertex_at.
    Intros.
    sep_apply (data_at_minus1_address sh' (Z2val (heapgraph_block_header g' a)) (heapgraph_block_ptr g' a)).
    Intros.
    forward.
    clear H37.
    ...
    rewrite data_at_tarray_value_split_1 by assumption.
    Intros.
    assert_PROP (
      force_val (sem_add_ptr_int int_or_ptr_type Signed (heapgraph_block_ptr g' a) (vint 0))
    ...
    forward.
    clear H38.
    sep_apply (field_at_data_at_cancel sh' int_or_ptr_type nv (heapgraph_block_ptr g' a)).
    gather_SEP
    ...
    rewrite <- (lmc_heapgraph_block_ptr g' a (new_copied_v g to)) in *.
    subst g'.
    change
      (lgraph_mark_copied (lgraph_add_copied_v g a to) a (new_copied_v g to))
      with (lgraph_copy_v g a to)
      in *.
    remember
      (lgraph_copy_v g a to)
      as g' eqn:Heqg'.
    rewrite <- H30 in *.
    thaw FR.
    forward_call (nv).
    subst p_addr.
    remember
      (cut_thread_info t_info (Z.of_nat to) (heapgraph_block_size g a) Hi Hh)
      as t_info' eqn:Heqt_info'.
    unfold thread_info_rep.
    Intros.
    forward.
    remember
    ...
    gather_SEP
      ...
      (heap_rest_rep _ ).
    replace_SEP 0 (thread_info_rep sh (update_thread_info_arg t_info' lz nv H16) ti).
    {
      unfold thread_info_rep. simpl heap_head. simpl ti_heap_p.
      simpl ti_args. simpl ti_heap. clear Heqt_info'. entailer!.
    }
    remember
      (update_thread_info_arg t_info' lz nv H16)
      as t eqn:Heqt.
    subst t_info'.
    rename t into t_info'.
    rewrite H30 in H32.
    assert
      (forward_relation from to 0 (inl (inr a)) g g')
      as H38
      by (subst g'; constructor; assumption).
    assert
      ...
      as H41.
    {
      ...
      refine {|
        ...
        thread_info_relation__gen_size m := _;
        thread_info_relation__space_base m := _;
      |}.
      - easy.
      - now rewrite utiacti_gen_size.
      - now rewrite utiacti_space_base.
    }
    forward_if.
    {
      pose proof H41 as HH41.
      destruct H41 as [H41 H43 H44].
      replace
        fp
        with (heapgraph_generation_base g' from)
        by (subst fp g'; apply lcv_gen_start; assumption).
      replace
        (offset_val fn (heapgraph_generation_base g' from))
      ...
      replace
        n_addr
        with (next_address t_info' to)
        by (subst n_addr; rewrite H41; reflexivity).
      forward_for_simple_bound
        n
      ...
      {
        pose proof (block_fields__range2 (heapgraph_block g a)).
        simpl in H45.
      ...
      }
      {
        ...
        assert
          (forward_loop from to (Z.to_nat (depth - 1)) [] g' g')
          by constructor.
        entailer!.
        now constructor.
      }
      {
        ...
        assert
          ...
          as H50
          by (rewrite Heqg', <- lcv_graph_has_gen; assumption).
        assert
          (heapgraph_has_block g' (new_copied_v g to))
          as H51
          by (rewrite Heqg'; apply lcv_heapgraph_has_block_new; assumption).
        forward_call (rsh, sh, gv, fi, ti, g3, t_info3, f_info, roots', outlier, from, to, depth - 1, (@inr Z _ (new_copied_v g to, i))).
        {
          apply prop_right.
          simpl.
          rewrite sub_repr, H30.
          do 4 f_equal.
          first [rewrite sem_add_pi_ptr_special' | rewrite sem_add_pl_ptr_special']; auto.
          - simpl.
            f_equal.
            erewrite fl_heapgraph_block_ptr; eauto.
            subst g'.
        ...
        }
        {
          ...
          - erewrite <- fl_block_fields; eauto. subst g'.
            ...
            now rewrite <- lmc_block_fields, lacv_vlabel_new.
        ...
        subst roots4.
        assert
          (heapgraph_generation_base g3 from = heapgraph_generation_base g4 from)
          as H53.
        {
          eapply fr_gen_start; eauto.
          erewrite <- fl_graph_has_gen; eauto.
        }
        rewrite H53.
        assert
        ...
        rewrite H57.
        assert
          (next_address t_info3 to = next_address t_info4 to)
          as H58.
        {
          unfold next_address.
          f_equal.
      ...
      2: {
        ...
        right.
        subst n g' from.
        now rewrite heapgraph_field_pairs__Zlength, lcv_vlabel_new.
      }
      ...
      replace
        (Z.to_nat depth)
        with (S (Z.to_nat (depth - 1)))
        by (rewrite <- Z2Nat.inj_succ; [f_equal|]; lia).
      now constructor.
    }
    assert
      (depth = 0)
    ...
    Exists
      (lgraph_copy_v g a to)
      (update_thread_info_arg t lz nv H16)
      (upd_bunch z f_info roots (inr (new_copied_v g to))).
    entailer!.
    simpl.
    rewrite <- Heqroot.
  ...
  remember (heapgraph_generation_sh g from) as fsh.
  remember
    (gen_size t_info from - space_remembered (nth_space t_info from))
    as gn.
  ...
  }
  {
    ...
    assert (P |-- (weak_derives P (valid_pointer (Vptr b i) * TT) && emp) * P). {
      ...
      simpl GC_Pointer2val. cancel. }
    replace_SEP 1 ((weak_derives P (valid_pointer (Vptr b i) * TT) && emp) * P) by (entailer; assumption).
    Intros.
    clear H19.
    forward_call (fsh, fp, fn, (Vptr b i), P).
    Intros vret.
    destruct vret. (* is_from? *)
    {
      (* yes *)
      rewrite HeqP. Intros.
  ...
  sep_apply (graph_and_heap_rest_v_in_range_iff _ _ _ _ H H7 H19).
  Intros. rewrite <- Heqfp, <- Heqgn, <- Heqfn, Heqv0 in H21.
  destruct vv.
  2: {
    forward_if.
    {
  ...
  forward_if.
  {
    ...
    split; [|split; [|split; [|split]]]; try easy.
    {
      ...
      intuition idtac.
      {
        now apply lgd_graph_thread_info_compatible.
      }
      {
        now apply lgd_rgc.
      }
  ...
  assert_PROP (
    force_val (sem_add_ptr_int int_or_ptr_type Signed (heapgraph_block_ptr g' v') (vint 0))
  ...
  {
    now apply (graph_vertex_lgd_ramif g' v e (new_copied_v g to) n).
  }
  ...
  unfold thread_info_rep. Intros.
  assert
    (0 <= 0 < Zlength (ti_args t_info'))
    by (rewrite arg_size; rep_lia).
  gather_SEP
    (data_at _ _ _ _)
  ...
  rewrite H31 in H33.
  assert
    (forward_relation from to 0 (inr e) g g1)
    by (subst g1 g' v1 v'; constructor; assumption).
  ...
  assert
    (forward_condition g1 t_info' from to).
  {
    subst g1 g' t_info' from v'.
    assert
      (heapgraph_has_block (lgraph_copy_v g (dst g e) to) (dst g e))
      as HH.
    {
  ...
  assert (thread_info_relation t_info t_info').
  {
    ...
    |}.
    - easy.
    - now rewrite cti_gen_size.
    - now rewrite cti_space_base.
  }
  forward_if.
  2: {
  ...
  replace
    fp
    with (heapgraph_generation_base g1 from)
    by (subst fp g1 g'; apply lcv_gen_start; assumption).
  replace
    (offset_val fn (heapgraph_generation_base g1 from))
    with (limit_address g1 t_info' from).
  2: {
    subst fn gn.
    rewrite H57.
    rewrite <- nth_Znth'.
  ...
  replace
    n_addr
    with (next_address t_info' to)
    by (subst n_addr; rewrite H55; reflexivity).
  forward_for_simple_bound
    n'
    (EX i: Z, EX g3: HeapGraph, EX t_info3: thread_info,
    PROP
  ...
  - Intros.
    ...
    + Intros vret.
      ...
      assert
        (thread_info_relation t_info' t_info4)
        by (apply tir_trans with t_info3; assumption).
      assert
        (forward_loop
          from to (Z.to_nat (depth - 1))
          (sublist 0 (i + 1)
